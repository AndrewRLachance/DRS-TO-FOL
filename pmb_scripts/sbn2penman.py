...
transform one-line sbn to penman notation, put your sbn in sbn_template.txt, each sbn takes one line.
    python3 sbn2penman.py -s ./sbn_template.txt -o ./penman_template.txt
"""
...
from pmb_scripts.penman_model import pm_model
from pmb_scripts.graph_base import BaseEnum, BaseGraph
from typing import Any, Dict, Optional, Tuple, Union


from pmb_scripts.sbn_spec import (
    SBN_EDGE_TYPE,
    SBN_NODE_TYPE,
    SBNError,
    SBNSpec,
...
class SBNGraph(BaseGraph):
    ...
    def from_string(
        ...
        max_wn_idx = sum(
            1
            for sbn_line, _comment in lines
            if sbn_line.split() and SBNSpec.SYNSET_PATTERN.match(sbn_line.split()[0])
        ) - 1
        ...
        for sbn_line, comment in lines:
            ...
            while len(tokens) > 0:
                ...
                if tok_count == 0 and (
                    ...
                    if not tokens:
                        raise SBNError(
                            f"Missing box/operator index in line: {sbn_line}"
                        )
                    ...
                    # etc.
                    #
                    # This is a real box-creation connector in this parser's model.
                    if target.startswith("-") or target.startswith("+"):
                        if (box_index := self._try_parse_idx(target)) != -1:
                            raise SBNError(
                    ...
                    #   ALTERNATION <1
                    #
                    # This is a scopal operator attached to the active synset/line, not a
                    # new-box connector in the current parser representation.
                    elif target.startswith("<") or target.startswith(">"):
                        # PMB SBN uses angle scopes such as:
                        #   NEGATION <1
                        #   CONJUNCTION <2
                        #   EXPLANATION <1
                        #
                        # These can appear either:
                        #   1. after an active synset line, or
                        #   2. as standalone operator lines before any synset.
                        #
                        # For DRT export, representing them as BOX_BOX_CONNECT edges is more useful
                        # than attaching them to a synset as a binary predicate.
                        ...
                        raise SBNError(
                            f"Invalid box/operator target '{target}' in line: {sbn_line}"
                        )
                elif (
                    (is_role := (
                        token in SBNSpec.ROLES
                        or token.endswith("Of")
                        or re.fullmatch(r"[A-Z][A-Za-z-]*", token)
                    ))
                    or token in SBNSpec.DRS_OPERATORS
                ):
                    if not tokens:
                        raise SBNError(
                    ...
                    #   NEGATION <1
                    #
                    # These are not synset offsets. Treat the marker as a constant-like target
                    # so the graph remains well-formed. The DRS exporter can later skip or
                    # interpret these specially.
                    if SBNSpec.SCOPE_INDEX_PATTERN.match(target):
                    ...
                    # Normal synset-relative role target:
                    #   Agent -1
                    #   Theme +2
                    #   Time -3
                    elif SBNSpec.ROLE_INDEX_PATTERN.match(target):
                        idx = self._try_parse_idx(target)
                        active_id = self._active_synset_id
                        target_idx = active_id[1] + idx
                        to_id = (active_id[0], target_idx)

                        # Important: max_wn_idx should ideally be the number of synset lines - 1,
                        # not len(lines) - 1, because SBN can contain standalone operator lines.
                    ...
                    token.isupper()
                    and tokens
                    and (tokens[0].startswith("<") or tokens[0].startswith(">"))
                ):
                    target = tokens.pop(0)
    ...
    def _try_parse_idx(possible_idx: str) -> int:
        try:
            if possible_idx.startswith("<") or possible_idx.startswith(">"):
                possible_idx = possible_idx[1:]
...
if __name__ == '__main__':
    ...
    penman_path = args.output_file
    dot_path = args.dot_file

    with open(sbn_path, "r", encoding="utf-8") as f:
        sbn_data = f.readlines()
        with open(penman_path, "w", encoding="utf-8") as w:
            for s in sbn_data:
                sbn_graph_n = SBNGraph().from_string(s, is_single_line=True)
                penman = sbn_graph_n.to_penman_string()
    
                # write penman to file
                w.write(penman + "\n")

                # save dot file
                sbn_graph_n.to("dot", dot_path)
